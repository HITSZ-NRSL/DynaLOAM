...
class DynamicVoxelization(torch.autograd.Function):
    ...
    def forward(ctx, xyz: torch.Tensor,
        ...
        indices = xyz.new_empty([xyz.shape[0], 1], dtype=torch.int64)
        infos = havs_cuda.dynamic_voxelization(xyz, indices, voxel_size)
        return indices
...
dyn_vox_batch = DynamicVoxelization.apply
