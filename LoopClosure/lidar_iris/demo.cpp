...
int main(int argc, char *argv[])
{
    ...
    //kitti pose xx.txt
    auto gt = getGTFromPose("/media/eric/Elements SE/data/kitti_odom/dataset/poses/"+seq +".txt");
    ...
    for(int i =0; i <=N-1 ;i++)
    {
        ...
        // kitti velodyne bins
        std::string filename = "/media/eric/Elements SE/data/kitti_odom/dataset/sequences/" + seq + "/velodyne/" + ss.str() + ".bin";
        ...
        std::vector<int> ixi;
        std::vector<float> ixf;
        kdtree.radiusSearch(traj_vec[i], 5, ixi, ixf);
